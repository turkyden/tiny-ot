...
class WebSocketJSONStream extends Duplex {
  ...
  _write(object: any, encoding: string, callback: (error?: Error | null) => void) {
    let json;
  ...
  _send(json: string, callback: (error?: Error | null) => void) {
  ...
  _closeWebSocket(code: number, reason?: string, callback?: (error?: Error | null) => void) {
    switch (this.ws.readyState) {
      ...
      default: {
        process.nextTick(() =>
          callback && callback(new Error(`Unexpected readyState: ${this.ws.readyState}`))
        );
        break;
      }
    }
